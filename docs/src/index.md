...
A Julia implementation of the algebraic approach to timescale separation
...
Parameter Values (TFPVs), together with their slow manifolds and the computation
of the corresponding reductions for ODE system with polynomial right-hand side.
...
parameter. 
Due to [goeke2013a,goeke2014,goeke2015](@cite) we can compute the reduction for
this system, i.e. its limit for ``\varepsilon \to 0``, in the sense of Tikhonov
[tikhonov1952](@cite) and Fenichel [fenichel1979](@cite) using methods from
commutative algebra and algebraic geometry. 
Note that this yields all possible timescale separations of _rates_ and not
just _components_ as in the classical approach.
...
Roughly speaking, this method checks for all possible slow-fast separations of rates
``\pi^\star`` whether there exists an irreducible component of
``\mathcal{V}(f^{(0)})`` with the desired dimension ``s``, since these
correspond to the potential slow manifolds on which a reduction is defined.
...
``x_0\in\mathbb{R}^n`` for which ``f(x_0,\pi')`` and the determinants of
...
consider conditions for the parameters and not the state variables ``x``. 
For this, we compute the elimination ideal ``I_{\pi}:=I\cap\mathbb{R}[\pi]``. 
Given a Gröbner basis ``G`` of ``I`` w.r.t. an elimination ordering for ``x``,
``I_{\pi}`` is generated by ``G\cap\mathbb{R}[\pi]``,
i.e. all polynomials in ``G`` not containing the state variables ``x``.
Then, every TFPV must result in the vanishing of each polynomial in
...
Let ``r=n-s``.
We need to find a product decomposition for ``f^{(0)}``, i.e.
``\psi(x)\in\mathbb{R}[x]^{r\times1}`` and ``P(x)\in\mathbb{R}(x)^{n\times r}``
admitting
```math
f^{(0)}(x) = P(x)\psi(x)
```
in an open Zariski neighbourhood  ``\mathcal{U}_{x_0}``,
